...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.send_create_signal('tracker', ['Grant'])
        
        default_id = 1
        if not db.dry_run:
            unknown = orm.Grant.objects.create(full_name='unknown', short_name='unknown', slug='x')
            default_id = unknown.id

        # Adding field 'Topic.grant'
        db.add_column('tracker_topic', 'grant', self.gf('django.db.models.fields.related.ForeignKey')(default=default_id, to=orm['tracker.Grant']), keep_default=False)


    def backwards(self, orm):
        
        # Deleting model 'Grant'
        db.delete_table('tracker_grant')

        # Deleting field 'Topic.grant'
        db.delete_column('tracker_topic', 'grant_id')
