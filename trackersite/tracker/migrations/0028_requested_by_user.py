...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for ticket in orm.Ticket.objects.all():
            User = orm['auth.User']
            try:
                user = User.objects.get(username=ticket.requested_by)
            except User.DoesNotExist:
                user = None
            
            ticket.requested_user = user
            if user == None:
                ticket.requested_text = ticket.requested_by
            else:
    ...
    def backwards(self, orm):
        for ticket in orm.Ticket.objects.all():
            if ticket.requested_user != None:
                ticket.requested_by = ticket.requested_user.username
            else:
                ticket.requested_by = ticket.requested_text
            ticket.save()
